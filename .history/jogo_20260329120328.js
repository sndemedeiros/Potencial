...
function runAnimation() {
    ...
    if (currentIndex !== 0) {
        currentIndex = 0;
        updateAll();
        setTimeout(runAnimation, 200); // Pequeno delay para resetar e iniciar
        return;
    }
...
function animateParticle(currentTime) {
    const elapsed = currentTime - animStartT;
    let progress = Math.min(1, elapsed / ANIM_DUR);

    animPosFloat = progress * (NUM_POINTS - 1);
    drawField();
...
// ═══════════════════════════════════════════════════════════
//  DESENHO DO CAMPO E PARTÍCULA NO CANVAS
// ═══════════════════════════════════════════════════════════
function drawField() {
    ...
    // Placas
    ctx.fillStyle = '#38bdf8'; ctx.fillRect(pX0 - 24, plateY, 24, plateH);
    ctx.fillStyle = '#fff'; ctx.font = 'bold 16px sans-serif'; ctx.textBaseline = 'middle'; ctx.textAlign = 'center';
    ctx.fillText('+', 12, H / 2);
    ctx.fillStyle = '#bfdbfe'; ctx.font = '10px monospace'; ctx.textBaseline = 'alphabetic'; ctx.textAlign = 'center';
    ctx.fillText('100 V', 12, plateY - 6);

    ctx.fillStyle = '#f43f5e'; ctx.fillRect(pX1, plateY, 24, plateH);
    ctx.fillStyle = '#fff'; ctx.font = 'bold 16px sans-serif'; ctx.textBaseline = 'middle'; ctx.textAlign = 'center';
    ...
    ctx.lineCap = 'butt';
    ctx.fillStyle = '#475569'; ctx.font = '10px monospace'; ctx.textAlign = 'left'; ctx.textBaseline = 'alphabetic';
    ctx.fillText('→ E', pX0 + 4, plateY + plateH + 14);

    // Equipotenciais
    [80, 60, 40, 20].forEach(Veq => {
        const xEq = pX0 + (pX1 - pX0) * (1 - Veq / V_MAX);
        ctx.save(); ctx.setLineDash([4, 4]); ctx.strokeStyle = '#164e63'; ctx.lineWidth = 1;
        ctx.beginPath(); ctx.moveTo(xEq, plateY - 4); ctx.lineTo(xEq, plateY + plateH + 4); ctx.stroke();
        ctx.restore();
        ctx.fillStyle = '#22d3ee'; ctx.font = '9px monospace'; ctx.textAlign = 'center'; ctx.textBaseline = 'alphabetic';
    ...
    for (let i = 0; i < NUM_POINTS; i++) {
        ...
        const visited = charState[selectedChar].visited[i];

        if (isActive) {
            const Vi = computeV(i), Ui = computeU(ch.charge, Vi), Ki = computeK(ch.charge, i);
            const tagW = 170, tagH = 48;
            const tagX = Math.min(Math.max(px - tagW / 2, pX0), pX1 - tagW);
            const tagY = pY - 112;

            ctx.fillStyle = '#1e293b'; ctx.strokeStyle = '#475569'; ctx.lineWidth = 1; ctx.setLineDash([]);
            ...
            ctx.fillStyle = '#f97316'; ctx.fillText('U = ' + fmtEV(Ui), px, tagY + 22);
            ctx.fillStyle = Ki >= 0 ? '#22c55e' : '#f43f5e'; ctx.fillText('K = ' + fmtEV(Ki), px, tagY + 35);
            ...
            const ms = 4;
            ctx.strokeStyle = visited ? '#475569' : '#1e293b'; ctx.lineWidth = 1.5; ctx.setLineDash([]);
            ctx.beginPath();
            ctx.moveTo(px - ms, pY - ms); ctx.lineTo(px + ms, pY + ms);
            ctx.moveTo(px + ms, pY - ms); ctx.lineTo(px - ms, pY + ms);
            ctx.stroke();
            ctx.fillStyle = visited ? '#475569' : '#334155'; ctx.font = '10px monospace';
    ...
    // Partícula animada
    if (animating) {
        const px = pX0 + animPosFloat * step;
        const Vcurr = computeVcont(animPosFloat / (NUM_POINTS - 1));
        const Ucurr = computeU(ch.charge, Vcurr);
        const Kcurr = computeKcont(ch.charge, animPosFloat / (NUM_POINTS - 1));

        const tagW = 170, tagH = 48;
        const tagX = Math.min(Math.max(px - tagW / 2, pX0), pX1 - tagW);
        const tagY = pY - 112;

        ctx.fillStyle = '#1e293b'; ctx.strokeStyle = '#0284c7'; ctx.lineWidth = 1; ctx.setLineDash([]);
        ctx.beginPath(); ctx.roundRect(tagX, tagY, tagW, tagH, 6); ctx.fill(); ctx.stroke();

        ctx.font = '10px monospace'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
        ctx.fillStyle = '#38bdf8'; ctx.fillText('V = ' + Vcurr.toFixed(1) + ' V', px, tagY + 9);
        ctx.fillStyle = '#f97316'; ctx.fillText('U = ' + fmtEVf(Ucurr), px, tagY + 22);
        ctx.fillStyle = Kcurr >= 0 ? '#22c55e' : '#f43f5e'; ctx.fillText('K = ' + fmtEVf(Kcurr), px, tagY + 35);
    ...
    // seta de força
    const pxF = animating ? pX0 + animPosFloat * step : pX0 + currentIndex * step;
    const Kcheck = animating ? Kcurr : computeK(ch.charge, currentIndex);
    if (Kcheck >= 0 || !animating) {
        const dir = ch.charge > 0 ? 1 : -1;
        const ax0 = pxF + dir * 22, ax1 = pxF + dir * 60, ay = pY - 28;
    ...
    ctx.fillStyle = '#334155'; ctx.font = '9px monospace'; ctx.textAlign = 'left'; ctx.textBaseline = 'alphabetic';
        ...
        questions = [
            {
               ...
               O que acontece com U e K?`,
                ...
                feedback: `U = (−1e)·V. Quando V cai de 100 a 60, U vai de −100 a −60 eV: <b>U aumenta</b>.<br>
                   Para manter K + U constante, K <b>diminui</b>.`,
                isSynth: false
            },
            {
